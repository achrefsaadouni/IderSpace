...
import InvitationHeader from "../account/invitationHeader";
import {getUserInvitations} from "../../store/actions/profileActions";
import Spinner from "../common/Spinner";
import axios from "axios";
import { withRouter } from "react-router";
import Publications from "../account/publications";
...
class Header extends Component {
    constructor(props) {
        ...
        this.state = {
            etat: false,
            listInvitation: this.props.invitations
        }
    }
    ...
    onClickInvitation = (idUser , response) => {
        console.log(response);
        if (!this.state.etat){
            var {invitations} = this.props;
            this.setState({
                etat:true,
        ...
        axios({
            ...
            var newInvitation = this.state.listInvitation.filter(inv => inv._id !== idUser);
            this.setState({
                listInvitation: newInvitation,
                etat:true
            })
    ...
    clickButtonResearch = () => {
        var name = (document.getElementById("inputresearchFiends").value);
        this.props.history.push("/profile/search/"+name);
    }

    render() {
        const {user, invitations} = this.props;
        if (invitations === null) {
            return <Spinner/>
        }
        let invitationsInterface;
        if (!this.state.etat) {
            invitationsInterface = invitations.map(user => (
                <InvitationHeader onClickInvitation={this.onClickInvitation.bind(this)}  key={user._id} user={user}/>
            ));
        } else {
            invitationsInterface = this.state.listInvitation.map(user => (
                <InvitationHeader onClickInvitation={this.onClickInvitation.bind(this)}  key={user._id} user={user}/>
            ));
        }
        return (
                    ...
                    <div className="header-content-wrapper">
                        <div className="search-bar w-search notification-list friend-requests">
                            <div className="form-group with-button">
                                <input
                                    ...
                                    type="text"
                                    id="inputresearchFiends"
                                />
                                <button onClick={this.clickButtonResearch.bind(this)}>
                        ...
                        <div className="control-block">
                            <div className="control-icon more has-items">
                                ...
                                <div className="label-avatar bg-blue">{invitations.length}</div>
