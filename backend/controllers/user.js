...
const FriendsRequests = require("../Models/friendsRequests");
...
exports.refreshLinkedIn = (req, res, next) => {
    let fetchedUser;
    User.findById({_id: req.userData.userId})
        .then(user => {
            if (!user) {
        ...
        .then(result => {
            if (result.linkedin !== "") {
                config.rootProfiles.push(fetchedUser.linkedin);
                config.idUser = fetchedUser.id;
                scrapping(config)
                    .then(r => {
                        return r;
...
exports.searchUsers = (req, res, next) => {
    ...
    User.find({username: {$regex: name, $options: 'i'}}).then(re => {
    return res.status(200).json({
        re
    });
    })
    ...
    User.findById({_id: req.userData.userId})
        ...
        .then(() => {
            if (req.body.responseInvitation === true) {
                console.log('dkhal lel true');
                fetchedUser.friends.push(req.body.idInviter);
                FriendsRequests.find({
                    'sender': {$eq: req.body.idInviter},
                    'receiver': {$eq: req.userData.userId}
                }).then(request => {
                    console.log(req.body.idInviter);
                    request[0].etat = 'accepted';
                    fetchedFriendsRequest = request[0];
                    fetchedFriendsRequest.save().then(() => {
                        fetchedUser.save().then(result => {
                            res.status(200).json({
                                fetchedUser
                            });
...
exports.getInvitations = (req, res, next) => {
    ...
    User.findById({_id: req.userData.userId})
        ...
        .then(() => {
            FriendsRequests.find({'receiver': {$eq: req.userData.userId}, 'etat': {$eq: 'waiting'}}).then(re => {
                if (re.length !== 0){
                re.map(e => {
                    User.find({'_id': {$in: e.sender}}).then((senderUser) => {
...
exports.sendInvitation = (req, res, next) => {
    ...
    User.findById({_id: req.userData.userId})
        ...
        .then(() => {
            const friendsRequests = new FriendsRequests({
                sender: req.userData.userId,
                receiver: req.body.invitedUser
            });
            friendsRequests.save().then(() => {
                return res.status(200).json('invitation sended')
            })
        })
        .catch(err => {
            console.log(err);
